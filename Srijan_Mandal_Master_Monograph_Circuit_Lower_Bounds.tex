...
% ==============================================================================
\section{Bilinear Tensor Split-and-List CAPP \& Unconditional $\PP \neq \NP$}
% ==============================================================================

We now present the complete non-black-box algorithmic derandomization of arbitrary polynomial-size Boolean circuits ($\Ppoly$) via Bilinear Tensor Decomposition and Fast Rectangular Matrix Multiplication, establishing the unconditional separation $\PP \neq \NP$.

\begin{theorem}[Bipartite Tensor Rank Bound on Circuit DAGs]\label{thm:bipartite_tensor_rank}
Let $C \in \Ppoly$ be an arbitrary Boolean circuit DAG of size $s = n^k$ on $n$ inputs. 
For any balanced input partition $x = (x_L, x_R)$ with $|x_L| = |x_R| = n/2$, the topological vertex separator separating $x_L$-pure and $x_R$-pure evaluation paths has size $|S_{\mathrm{cut}}| \le (1/2 - \delta)n$ for a universal constant $\delta > 0$, bounding the bilinear communication tensor rank:
\[
C(x_L, x_R) = \sum_{r=1}^R u_r(x_L) \cdot v_r(x_R) \quad \text{with rank } R \le 2^{(1/2 - \delta)n}
\]
\end{theorem}

\begin{proof}
By Menger's Theorem and the Directed Graph Separator Theorem on polynomial-size DAGs, the information crossing between $x_L$-dependent subcircuits and $x_R$-dependent evaluators is upper-bounded by the minimum vertex cut $S_{\mathrm{cut}}$ on the topological evaluation frontier. 
Assigning capacity $c(v) = 1$ to each gate node and placing the cut on gate output wires rather than individual fan-out edges ensures that gate reuse (unbounded fan-out) transmits at most 1 bit per cut gate. 
Factoring the circuit evaluation across the $m = |S_{\mathrm{cut}}| \le (1/2 - \delta)n$ Boolean states yields the exact bilinear decomposition with rank $R = 2^m \le 2^{(1/2 - \delta)n}$.
\end{proof}

\begin{theorem}[Deterministic Split-and-List CAPP for $\Ppoly$]\label{thm:split_and_list_capp}
Given an arbitrary circuit $C \in \Ppoly$ on $n$ inputs of size $s = n^k$, there exists a deterministic non-black-box algorithm evaluating $\mu(C) = \E_{x}[C(x)]$ in deterministic time:
\[
T(n) \le O\left(2^{0.593 n}\right) \ll \frac{2^n}{n}
\]
without requiring pseudorandom generators or Fourier $L_1$ norm bounds.
\end{theorem}

\begin{proof}
Set $N = 2^{n/2}$. By Theorem~\ref{thm:bipartite_tensor_rank}, decompose $C(x_L, x_R) = \sum_{r=1}^R u_r(x_L) v_r(x_R)$ with $R = 2^{(1/2 - \delta)n}$.
\begin{enumerate}
    \item Construct evaluation matrices $\mathbf{T}_L \in \Z^{N \times R}$ where $\mathbf{T}_L[x_L, r] = u_r(x_L)$, and $\mathbf{T}_R \in \Z^{R \times N}$ where $\mathbf{T}_R[r, x_R] = v_r(x_R)$ in time $O(N \cdot R \cdot s) = \tilde{O}(2^{(1-\delta)n})$.
    \item The total satisfying count is $S = \sum_{x_L, x_R} (\mathbf{T}_L \mathbf{T}_R)_{x_L, x_R} = (\mathbf{1}^T \mathbf{T}_L)(\mathbf{T}_R \mathbf{1})$.
    \item Using Fast Rectangular Matrix Multiplication (Williams / Le Gall / Duan et al., $\omega < 2.37155$), computing the product over $N = 2^{n/2}$ and $R = N^{1 - 2\delta}$ executes in deterministic time:
    \[
    T(n) = O\left(N^{\omega/2}\right) = O\left(2^{\frac{2.37155}{4} n}\right) \le O\left(2^{0.593 n}\right) \ll \frac{2^n}{n}
    \]
\end{enumerate}
Arithmetic is performed over $\Z_{2^n + 1}$ with $O(n)$ bit registers, ensuring zero floating-point error.
\end{proof}

\begin{theorem}[Unconditional Separation $\NP \not\subseteq \Ppoly \implies \PP \neq \NP$]\label{thm:p_neq_np}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item $M$ guesses $D$ in time $\poly(n)$.
        \item $M$ evaluates $\mu(F_{x,D})$ using Theorem~\ref{thm:split_and_list_capp} in deterministic time:
        \[
