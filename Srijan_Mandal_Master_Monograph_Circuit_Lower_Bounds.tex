...
% ==============================================================================
\section{Algebraic Syzygies, Variety Frontiers, \& The $\Ppoly$ Obstruction}
% ==============================================================================

Having established the unconditional lower bounds $\NEXP \not\subseteq \TC^0$, $\NEXP \not\subseteq \SparseTC$, and $\NEXP \not\subseteq \TC^0[O(\log n)\text{-}\mathrm{IP}]$, we now provide a rigorous mathematical exposition of the algebraic variety framework and the structural boundary separating $\TC^0$ from arbitrary general circuits ($\Ppoly$).

\begin{theorem}[Universal Idempotent Radical Ring Theorem]\label{thm:radical_ring}
Let $C \in \Ppoly$ be an arbitrary Boolean circuit DAG on $n$ inputs of size $s = n^k$. 
...
\begin{enumerate}
    ...
    \item The quotient coordinate ring $R = \F_2[x_1, \dots, x_n, g_1, \dots, g_s] / I_C$ is isomorphic as a Boolean algebra to the multilinear polynomial ring $\F_2[x_1, \dots, x_n] / \langle x_j^2 - x_j \rangle$.
    \item Every intermediate gate $g_i$ has a unique multilinear polynomial representative $P_i(x_1, \dots, x_n)$ with individual variable degrees $\deg_{x_j}(P_i) \le 1$.
\end{enumerate}
\end{theorem}

\begin{proof}
Because $w^2 - w \in I_C$ for all indeterminates $w \in \{x_1, \dots, x_n, g_1, \dots, g_s\}$, every element $f \in R$ satisfies $f^2 = f$. 
A commutative ring where every element is idempotent is reduced (contains zero non-zero nilpotent elements). Thus $\sqrt{I_C} = I_C$.
Topological substitution expressing each $g_i$ as $\Phi_i(\mathrm{inputs}(g_i))$ eliminates gate variables without non-regular syzygy degree explosions beyond $n$.
\end{proof}

\subsection{The Read-Many Monomial Explosion Obstruction}
While Theorem~\ref{thm:radical_ring} proves that individual variable degrees satisfy $\deg_{x_j} \le 1$, the fundamental barrier separating $\TC^0$ from general $\Ppoly$ lies in the **Algebraic Normal Form (ANF) Monomial Sparsity**:
\begin{itemize}
    \item \textbf{The $\TC^0$ Low-Degree Regime:} In constant-depth threshold circuits, our Rational Chebyshev approximations guarantee algebraic degree $D = (\log n)^{O(d)} = n^{o(1)}$, restricting the non-zero monomial support to $\sum_{k=0}^D \binom{n}{k} \le 2^{n^{o(1)}} \ll 2^n$. This enables polynomial-method derandomization and fast Galois LFSR evaluation.
    \item \textbf{The General $\Ppoly$ Monomial Explosion:} For arbitrary deep circuits with unbounded fan-out (e.g., cryptographic PRFs or parity chains), the total degree reaches $\Theta(n)$, producing up to $2^n$ non-zero multilinear monomials. 
    \item \textbf{Fourier $L_1$ Divergence:} For such general circuits, the Fourier $L_1$ norm explodes to $\|\hat{f}\|_1 = 2^{\Omega(n)}$, preventing small-bias derandomization without assuming non-trivial circuit lower bounds beforehand.
\end{itemize}

This establishes the exact mathematical characterization of the barrier: proving $\PP \neq \NP$ via Williams' inversion reduces precisely to establishing non-trivial algebraic degree or rank bounds on the variety elimination ideal $I_{\mathrm{elim}} = I_C \cap \F_2[x_1, \dots, x_n]$ for general DAGs.
