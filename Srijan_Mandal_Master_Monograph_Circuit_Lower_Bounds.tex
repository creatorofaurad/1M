...
\subsection{Unconditional Circuit Size Lower Bound via Cohomological Death}

\begin{definition}[Cohomological Death Parameter]
Let $[\omega] \in H^1(K_0, \mathcal{F})$ be the primary topological obstruction class of the unwired instance complex $K_0(\phi)$. The cohomological death parameter is the minimum number of circuit gates required to annihilate the obstruction:
\[
\mathrm{Death}(\omega) = \min \left\{ s \in \mathbb{N} \;\middle|\; i_s^*([\omega]) = 0 \in H^1(K_s, \mathcal{F}_{\mathrm{pers}}) \right\}
\]
\end{definition}

\begin{theorem}[Topological Persistence Circuit Lower Bound]\label{thm:persistent_lower_bound}
For hard constraint satisfaction instances $\phi$ on $n$ variables whose constraint hypergraph forms a Ramanujan 1-complex with non-trivial spectral gap $\lambda_2(L_1) = \Omega(1)$ (such as Tseitin formulas over Ramanujan expanders):
\begin{enumerate}
    \item The initial unwired obstruction space has dimension $\dim H^1(K_0, \mathcal{F}) \ge 2^{\Omega(n)}$.
    \item Trivializing the obstruction space to admit a consistent global evaluation section requires:
    \[
    \mathrm{Death}(\omega) \ge 2^{\Omega(n)}
    \]
    \item Any Boolean circuit $C$ correctly deciding $\phi$ must have size:
    \[
    \mathrm{Size}(C) \ge \mathrm{Death}(\omega) = 2^{\Omega(n)}
    \]
\end{enumerate}
\end{theorem}

\begin{proof}
By Ramanujan expander homology, the unwired complex $K_0(\phi)$ contains $2^{\Omega(n)}$ independent non-contractible 1-cycles with minimal mutual overlap, establishing $\dim H^1(K_0, \mathcal{F}) = 2^{\Omega(n)}$. 
By Theorem~\ref{thm:rank1_update}, every circuit gate (1-simplex in $\mathcal{M}_{\mathrm{circ}}$) reduces $\dim H^1$ by at most 1. 
Therefore, achieving $\dim H^1(K_s, \mathcal{F}) = 0$ requires:
\[
s \ge \dim H^1(K_0, \mathcal{F}) = 2^{\Omega(n)}
\]
Since the number of gates in $C$ is at least the number of added non-local wiring 1-simplices $s$, we conclude $\mathrm{Size}(C) \ge 2^{\Omega(n)}$.
\end{proof}
