...
\begin{itemize}
    ...
    \item By Theorem~\ref{thm:deterministic_capp}, $M$ runs the deterministic $\CAPP$ algorithm on $F_{x,D}$ over the explicit $\epsilon$-biased space $X \subseteq \{0,1\}^m$ in time $T_{\mathrm{eval}} = \poly(m) = \poly(n)$.
